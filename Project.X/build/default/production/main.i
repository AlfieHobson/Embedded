...
_Bool RTC_isDay(struct Time time);
...
# 1 "./EEPROM.h" 1
struct Limits EEPROM_limitsRead (_Bool day);
void EEPROM_limitsWrite (struct Limits limits, _Bool day);
_Bool EEPROM_isEmpty(void);
# 12 "main.c" 2
...
int main(int argc, char** argv) {
    ...
    _Bool emptyEEPROM = EEPROM_isEmpty();
    if (emptyEEPROM)
    {
...
void checkTemp() {
    ...
    _Bool day = RTC_isDay(currentTime);
...
void systemReset(void) {
    ...
    {
        currentTime = RTC_getTime();
        if (currentTime.sec2 != currentSecond) {
            LCD_thirdLine();
            LCD_writeChar(counter + 48);
            currentSecond = currentTime.sec2;
            counter--;
        }
    }
    LCD_clear();
